...
const StageSelectScreen = () => {
  ...
  const ids = stageIdsOf(WORLD.number)
  const cards = ids.map((id, i) => {
    ...
    return {
      ...
      boss: isBossStage(id),
      bossHint: stage?.best ? `${moveLimit(stage.best)}번 안에 도착` : undefined,
    }
  })
      ...
      <section className="scroll-area flex min-h-0 flex-1 flex-col gap-6 rounded-[22px] border border-line bg-base-bg p-5 sm:gap-8 sm:p-9">
        <header className="flex items-start justify-between gap-4">
          <div className="flex items-start gap-4">
            <Button variant="icon" onClick={() => goTo('title')} aria-label="타이틀로">
              ←
            </Button>
            <div className="flex flex-col gap-1">
              ...
              <span className="text-2xl tracking-tight sm:text-3xl">{WORLD.name}</span>
            </div>
          </div>
