...
- **확대 상한:** 화면에 그려지는 칸 폭은 145px을 넘지 않는다. 구역이 작으면 화면을 채우려다 지나치게 확대되어서 둔 값이고, 남는 자리에는 구역 밖 지형이 더 보인다. 배율을 고르는 `Math.min`에 항목 하나로 들어가서 확대만 막고, 구역이 화면보다 커서 배율이 이미 작을 때는 걸리지 않는다. 폰에서는 칸 폭이 52~89px이라 걸리지 않는다
